...
/**
 * @author vill on 16/1/11 09:30. redis 数据连接池工厂
 */
public class RedisClusterFactory implements RedisClientFactory<ClusterRedisClient> {
	...
	/**
	 * 集群模式下 jedisCluster 不用手动返还 jedis client jedisCluster 有自动返还client策略
	 *
	 * @param client
	 */
	@Override
	...
	public void build() {
		...
		/**
		 * poolConfig.setTestOnReturn(true); //Idle时进行连接扫描
		 * poolConfig.setTestWhileIdle(true); //表示idle object evitor两次扫描之间要sleep的毫秒数
		 * poolConfig.setTimeBetweenEvictionRunsMillis(30000); //表示idle object
		 * evitor每次扫描的最多的对象数 poolConfig.setNumTestsPerEvictionRun(10);
