	...
	private static String getChildOfTree(Menu menuItem, int level, List<Menu> menuList) {
		...
		if (level > 0) {// level 为0是功能菜单
			...
			if (menuItem.getHref() != null && menuItem.getHref().length() > 0) {// 如果是子节点
				...
				if (menuItem.getTarget() != null && !menuItem.getTarget().equals("")) {
					menuString.append("<li><a class=\"J_menuItem\"  href=\"" + href + "\" target=\""
							+ menuItem.getTarget() + "\" " + "><i class=\"fa " + menuItem.getIcon()
							+ "\"></i>&nbsp;&nbsp;" + menuItem.getName() + "</a></li>\n");
				} else {
					menuString.append("<li><a class=\"J_menuItem\" href=\"" + href + "\"><i class=\"fa "
							+ menuItem.getIcon() + "\"></i>&nbsp;&nbsp;" + menuItem.getName() + "</a></li>\n");
				}
		...
		if ((menuItem.getHref() == null || menuItem.getHref().trim().equals("")) && menuItem.getIsShow().equals("1")) {// 如果是父节点且显示
			...
			if (level > 0) {// 不是功能菜单
				...
				menuString.append("<a  data-toggle=\"collapse\" data-parent=\"#" + menuItem.getParentId()
						+ "\" class=\"collapsed\" href=\"#" + menuItem.getId() + "\"><i class=\"fa "
						+ menuItem.getIcon() + "\"></i>&nbsp;&nbsp;" + menuItem.getName()
						+ "<span class=\"pull-right fa fa-angle-toggle\"></span></a>\n");
				menuString.append("<ul id=\"" + menuItem.getId() + "\"" + " class=\"nav collapse\">\n");
