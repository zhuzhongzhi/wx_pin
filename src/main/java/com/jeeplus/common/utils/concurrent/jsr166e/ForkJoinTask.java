...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
	 ...
	 * implementation overview. ForkJoinTasks are mainly responsible for maintaining
	 * their "status" field amidst relays to methods in ForkJoinWorkerThread and
	 * ForkJoinPool.
	 *
	 * The methods of this class are more-or-less layered into (1) basic status
	 * maintenance (2) execution and awaiting completion (3) user-level methods that
	 * additionally report results. This is sometimes hard to see because this file
	 * orders exported methods in a way that flows well in javadocs.
	 */

	/*
	 * The status field holds run control status bits packed into a single int to
	 * minimize footprint and to ensure atomicity (via CAS). Status is initially
	 * zero, and takes on nonnegative values until completed, upon which status
	 * (anded with DONE_MASK) holds value NORMAL, CANCELLED, or EXCEPTIONAL. Tasks
	 * undergoing blocking waits by other threads have the SIGNAL bit set.
	 * Completion of a stolen task with SIGNAL set awakens any waiters via
	 * notifyAll. Even though suboptimal for some purposes, we use basic builtin
	 * wait/notify to take advantage of "monitor inflation" in JVMs that we would
	 * otherwise need to emulate to avoid adding further per-task bookkeeping
	 * overhead. We want these monitors to be "fat", i.e., not use biasing or
	 * thin-lock techniques, so use some odd coding idioms that tend to avoid them,
	 * mainly by arranging that every synchronized block performs a wait, notifyAll
	 * or both.
	 *
	 * These control bits occupy only (some of) the upper half (16 bits) of status
	...
	/**
	 * Tries to set SIGNAL status unless already completed. Used by ForkJoinPool.
	 * Other variants are directly incorporated into externalAwaitDone etc.
	 *
	...
	/**
	 * Table of exceptions thrown by tasks, to enable reporting by callers. Because
	 * exceptions are rare, we don't directly keep them with task objects, but
	 * instead use a weak ref table. Note that cancellation exceptions don't appear
	 * in the table, but are instead recorded as status values.
	 *
	 ...
	 * must use weak references and expunge them. We do so on each operation (hence
	 * full locking). Also, some thread in any ForkJoinPool will call
	 ...
	 * pool shutdown. Cancel is spec'ed not to throw any exceptions, but if it does
	 * anyway, we have no recourse during shutdown, so guard against this case.
	 */
	...
	/**
	 * Returns a rethrowable exception for the given task, if available. To provide
	 * accurate stack traces, if the exception was not thrown by the current thread,
	 * we try to create a new exception of the same type as the one thrown, but with
	 * the recorded exception as its cause. If there is no such constructor, we
	 * instead try to use a no-arg constructor, followed by initCause, to the same
	 * effect. If none of these apply, or any fail due to other exceptions, we
	 * return the recorded exception, which is still correct, although it may
	 ...
	 * running in, if applicable, or using the {@link ForkJoinPool#commonPool()} if
	 * not {@link #inForkJoinPool}. While it is not necessarily enforced, it is a
	 * usage error to fork a task more than once unless it has completed and been
	 * reinitialized. Subsequent modifications to the state of this task or any data
	 * it operates on are not necessarily consistently observable by any thread
	 * other than the one executing it unless preceded by a call to {@link #join} or
	...
	/**
	 * Returns the result of the computation when it {@link #isDone is done}. This
	 * method differs from {@link #get()} in that abnormal completion results in
	 * {@code RuntimeException} or {@code Error}, not {@code ExecutionException},
	 * and that interrupts of the calling thread do <em>not</em> cause the method to
	 * abruptly return by throwing {@code
	...
	/**
	 * Forks the given tasks, returning when {@code isDone} holds for each task or
	 * an (unchecked) exception is encountered, in which case the exception is
	 * rethrown. If more than one task encounters an exception, then this method
	 * throws any one of these exceptions. If any task encounters an exception, the
	 * other may be cancelled. However, the execution status of individual tasks is
	 * not guaranteed upon exceptional return. The status of each task may be
	 * obtained using {@link #getException()} and related methods to check if they
	 * have been cancelled, completed normally or exceptionally, or left
	 * unprocessed.
	 *
	 * @param t1
	 *            the first task
	 ...
	 * others may be cancelled. However, the execution status of individual tasks is
	 * not guaranteed upon exceptional return. The status of each task may be
	 * obtained using {@link #getException()} and related methods to check if they
	 * have been cancelled, completed normally or exceptionally, or left
	 * unprocessed.
	 ...
	 * holds for each task or an (unchecked) exception is encountered, in which case
	 * the exception is rethrown. If more than one task encounters an exception,
	 * then this method throws any one of these exceptions. If any task encounters
	 * an exception, others may be cancelled. However, the execution status of
	 * individual tasks is not guaranteed upon exceptional return. The status of
	 * each task may be obtained using {@link #getException()} and related methods
	 * to check if they have been cancelled, completed normally or exceptionally, or
	 * left unprocessed.
	 *
	 * @param tasks
	 ...
	 * execution of this task is suppressed. After this method returns successfully,
	 * unless there is an intervening call to {@link #reinitialize}, subsequent
	 * calls to {@link #isCancelled}, {@link #isDone}, and {@code cancel} will
	 * return {@code true} and calls to {@link #join} and related methods will
	 * result in {@code CancellationException}.
	 *
	 * <p>
	 * This method may be overridden in subclasses, but if so, must still ensure
	 * that these properties hold. In particular, the {@code cancel} method itself
	 * must not throw exceptions.
	 *
	 * <p>
	 * This method is designed to be invoked by <em>other</em> tasks. To terminate
	 * the current task, you can just return or throw an unchecked exception from
	 * its computation method, or invoke {@link #completeExceptionally(Throwable)}.
	 *
	...
	/**
	 * Returns {@code true} if this task completed without throwing an exception and
	 * was not cancelled.
	 *
	 * @return {@code true} if this task completed without throwing an exception and
	 *         was not cancelled
	 */
	 ...
	 * {@code CancellationException} if cancelled, or {@code null} if none or if the
	 ...
	 * tasks, or to force completion of tasks that would not otherwise complete. Its
	 * use in other situations is discouraged. This method is overridable, but
	 * overridden versions must invoke {@code super} implementation to maintain
	 * guarantees.
	 *
	 * @param ex
	 ...
	 * given value as the result of subsequent invocations of {@code join} and
	 * related operations. This method may be used to provide results for
	 * asynchronous tasks, or to provide alternative handling for tasks that would
	 * not otherwise complete normally. Its use in other situations is discouraged.
	 * This method is overridable, but overridden versions must invoke {@code super}
	 ...
	 * {@linkplain ForkJoinPool#isQuiescent is quiescent}. This method may be of use
	 * in designs in which many tasks are forked, but none are explicitly joined,
	 * instead executing them until all are processed.
	 */
	 ...
	 * reuse occurs when this task has either never been forked, or has been forked,
	 * then completed and all outstanding joins of this task have also completed.
	 * Effects under any other usage conditions are not guaranteed. This method may
	 * be useful when executing pre-constructed trees of subtasks in loops.
	 *
	 * <p>
	 * Upon completion of this method, {@code isDone()} reports {@code false}, and
	 * {@code getException()} reports {@code
	 * null}. However, the value returned by {@code getRawResult} is unaffected. To
	 * clear this value, you can invoke {@code
	 * setRawResult(null)}.
	 */
	...
	/**
	 * Returns {@code true} if the current thread is a {@link ForkJoinWorkerThread}
	 * executing as a ForkJoinPool computation.
	 *
	 * @return {@code true} if the current thread is a {@link ForkJoinWorkerThread}
	...
	/**
	 * Tries to unschedule this task for execution. This method will typically (but
	 * is not guaranteed to) succeed if this task is the most recently forked task
	 * by the current thread, and has not commenced executing in another thread.
	 * This method may be useful when arranging alternative local processing of
	 ...
	 * them, or zero if this thread is not operating in a ForkJoinPool. This value
	 * may be useful for heuristic decisions about whether to fork other tasks. In
	 * many usages of ForkJoinTasks, at steady state, each worker should aim to
	 * maintain a small constant surplus (for example, 3) of tasks, and to process
	 * computations locally if this threshold is exceeded.
	 *
	...
	/**
	 * Returns the result that would be returned by {@link #join}, even if this task
	 * completed abnormally, or {@code null} if this task is not known to have been
	 * completed. This method is designed to aid debugging, as well as to support
	 ...
	 * This method may also throw an (unchecked) exception to indicate abnormal
	 * exit. This method is designed to support extensions, and should not in
	 * general be called otherwise.
	 *
	 ...
	 * Conversely, this method may return null even if a task exists but cannot be
	 * accessed without contention with other threads. This method is designed
	 ...
	 * current thread but not yet executed, if the current thread is operating in a
	 ...
	 * not yet executed, if one is available, or if not available, a task that was
	 * forked by some other thread, if available. Availability may be transient, so
	 * a {@code null} result does not necessarily imply quiescence of the pool this
	 * task is operating in. This method is designed primarily to support
	 ...
	 * @param tag
	 *            the new tag value
	 * @return {@code true} if successful; i.e., the current value was equal to e
	 *         and is now tag.
	 * @since 1.8
	 */
	...
	/**
	 * Returns a new {@code ForkJoinTask} that performs the {@code run} method of
	 * the given {@code Runnable} as its action, and returns the given result upon
	 * {@link #join}.
	 *
	 * @param runnable
	 *            the runnable action
	...
	/**
	 * Returns a new {@code ForkJoinTask} that performs the {@code call} method of
	 * the given {@code Callable} as its action, and returns its result upon
	 ...
	 * @serialData the current run status and the exception thrown during execution,
