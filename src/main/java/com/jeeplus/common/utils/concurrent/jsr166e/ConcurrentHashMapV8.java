 ...
 * as {@link Hashtable}, and includes versions of methods corresponding to each
 * method of {@code Hashtable}. However, even though all operations are
 ...
 * prevents all access. This class is fully interoperable with {@code Hashtable}
 * in programs that rely on its thread safety but not on its synchronization
 * details.
 *
 * <p>
 ...
 * should ideally be side-effect-free. Bulk operations on {@link Entry} objects
...
public class ConcurrentHashMapV8<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	...
	public static interface ConcurrentHashMapSpliterator<T> {
		/**
		 * If possible, returns a new spliterator covering approximately one half of the
		 * elements, which will not be covered by this spliterator. Returns null if
		 * cannot be split.
		 */
	 ...
	 * constructor arguments referring to concurrencyLevel. We accept a loadFactor
	 ...
	 *
	 * <p>
	 * The view's {@code iterator} is a "weakly consistent" iterator that will never
	 ...
	 * @param defaultValue
	 *            the value to return if this map contains no mapping for the given
	 *            key
	 ...
	 * compatibility with class {@link Hashtable}, which supported this method prior
	 ...
	 * {@link #size} because a ConcurrentHashMapV8 may contain more mappings than
	 * can be represented as an int. The value returned is an estimate; the actual
	...
	/**
	 * Creates a new {@link Set} backed by a ConcurrentHashMapV8 from the given type
	 ...
	 *            the (estimated) number of elements needed for this operation to be
	 ...
	 *            executed in parallel
	 * @param transformer
	 *            a function returning the transformation for an element, or null if
	 ...
	 * @since 1.8
	 */
	public <U> void forEachEntry(long parallelismThreshold, Fun<Entry<K, V>, ? extends U> transformer,
			Action<? super U> action) {
		if (transformer == null || action == null)
			throw new NullPointerException();
	 ...
	 * @since 1.8
	 */
	public <U> U reduceEntries(long parallelismThreshold, Fun<Entry<K, V>, ? extends U> transformer,
			BiFun<? super U, ? super U, ? extends U> reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * @since 1.8
	 */
	public double reduceEntriesToDouble(long parallelismThreshold, ObjectToDouble<Entry<K, V>> transformer,
			double basis, DoubleByDoubleToDouble reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * @since 1.8
	 */
	public long reduceEntriesToLong(long parallelismThreshold, ObjectToLong<Entry<K, V>> transformer, long basis,
			LongByLongToLong reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	 ...
	 * @since 1.8
	 */
	public int reduceEntriesToInt(long parallelismThreshold, ObjectToInt<Entry<K, V>> transformer, int basis,
			IntByIntToInt reducer) {
		if (transformer == null || reducer == null)
			throw new NullPointerException();
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Set} of keys, in which additions
	...
	/**
	 * A view of a ConcurrentHashMapV8 as a {@link Collection} of values, in which
	...
	static final class EntrySetView<K, V> extends CollectionView<K, V, Entry<K, V>>
		...
		public boolean contains(Object o) {
			Object k, v, r;
			Entry<?, ?> e;
			return ((o instanceof Map.Entry) && (k = (e = (Entry<?, ?>) o).getKey()) != null && (r = map.get(k)) != null
	...
	static final class MapReduceEntriesTask<K, V, U> extends BulkTask<K, V, U> {
		...
		MapReduceEntriesTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesTask<K, V, U> nextRight, Fun<Entry<K, V>, ? extends U> transformer,
				BiFun<? super U, ? super U, ? extends U> reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
	...
	static final class MapReduceEntriesToDoubleTask<K, V> extends BulkTask<K, V, Double> {
		...
		MapReduceEntriesToDoubleTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesToDoubleTask<K, V> nextRight, ObjectToDouble<Entry<K, V>> transformer, double basis,
				DoubleByDoubleToDouble reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
	...
	static final class MapReduceEntriesToLongTask<K, V> extends BulkTask<K, V, Long> {
		...
		MapReduceEntriesToLongTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesToLongTask<K, V> nextRight, ObjectToLong<Entry<K, V>> transformer, long basis,
				LongByLongToLong reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
	...
	static final class MapReduceEntriesToIntTask<K, V> extends BulkTask<K, V, Integer> {
		...
		MapReduceEntriesToIntTask(BulkTask<K, V, ?> p, int b, int i, int f, Node<K, V>[] t,
				MapReduceEntriesToIntTask<K, V> nextRight, ObjectToInt<Entry<K, V>> transformer, int basis,
				IntByIntToInt reducer) {
			super(p, b, i, f, t);
			this.nextRight = nextRight;
			this.transformer = transformer;
	 ...
	 * The code is initialized via the counterHashCodeGenerator, but may be moved
	 * upon collisions.
	 */
