		...
		if (loginName != null) {
			...
			for (ChatHistory c : list) {
				...
				if ("friend".equals(c.getType())) {// 如果是个人信息
					...
					String msg = sender + Constants._msg_ + c.getReceiver() + Constants._msg_ + c.getMsg()
							+ Constants._msg_ + UserUtils.getByLoginName(sender).getPhoto() + Constants._msg_
							+ c.getType() + Constants._msg_ + sender + Constants._msg_ + c.getCreateDate().getTime();
					...
					String msg = groupId + Constants._msg_ + c.getReceiver() + Constants._msg_ + c.getMsg()
							+ Constants._msg_ + UserUtils.getByLoginName(sender).getPhoto() + Constants._msg_
							+ c.getType() + Constants._msg_ + sender + Constants._msg_ + c.getCreateDate().getTime();
					this.sendMessageToUser(loginName, msg);
				}
		...
		if (null != message && message.contains(Constants._msg_)) {// 发送消息
			...
			if ("group".equals(type)) {// 如果是群聊
				...
				for (LayGroupUser lgUser : layGroupUserlist) {
					...
					chat.setType("group");
					message = groupId + Constants._msg_ + lgUser.getUser().getLoginName() + Constants._msg_ + msg
							+ Constants._msg_ + avatar + Constants._msg_ + type + Constants._msg_ + senderName
							+ Constants._msg_ + datatime;
