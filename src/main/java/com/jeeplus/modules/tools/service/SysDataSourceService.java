...
public class SysDataSourceService extends CrudService<SysDataSourceMapper, SysDataSource> {
	...
	public List<DbTable> findTableList(SysDataSource sysDataSource) {
		if (sysDataSource.getDbDriver().equals("oracle.jdbc.driver.OracleDriver")) {
			return MultiDBUtils.init(sysDataSource).queryList(
			...
			return MultiDBUtils.init(sysDataSource)
					.queryList("SELECT t.table_name AS name,t.TABLE_COMMENT AS comments\n"
							+ "\t\t\tFROM information_schema.`TABLES` t\n"
							+ "\t\t\tWHERE t.TABLE_SCHEMA = (select database())\n" + "\t\t\tORDER BY t.TABLE_NAME",
							DbTable.class);
		} else if (sysDataSource.getDbDriver().equals("org.postgresql.Driver")) {
			return MultiDBUtils.init(sysDataSource).queryList(
					"SELECT   tablename as name,obj_description(relfilenode,'pg_class') as comments FROM   pg_tables  a, pg_class b  \n"
	...
	public List<DbTableColumn> findTableColumnList(SysDataSource sysDataSource, String tableName) {
		if (sysDataSource.getDbDriver().equals("oracle.jdbc.driver.OracleDriver")) {
			...
			return MultiDBUtils.init(sysDataSource).queryList(
					"SELECT a.attname as name, col_description(a.attrelid,a.attnum) as comments,format_type(a.atttypid,a.atttypmod) as jdbcType\n"
