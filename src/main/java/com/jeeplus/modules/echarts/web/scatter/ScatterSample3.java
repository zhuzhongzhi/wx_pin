...
public class ScatterSample3 {
	...
	public GsonOption getOption() {
		...
		option.tooltip().trigger(Trigger.axis).axisPointer().show(true).type(PointerType.cross).lineStyle()
				.type(LineType.dashed).width(1);
		option.legend("series1");
		option.toolbox().show(true).feature(Tool.mark, Tool.dataView, Tool.restore, Tool.saveAsImage);
		option.dataZoomNew().show(true).start(30).end(70);
		option.dataRange().min(0).max(100).orient(Orient.horizontal).x(30).y(Y.center).color("lightgreen", "orange")
				.splitNumber(5);
		option.grid().y2(80);
		...
		series1.tooltip()
				.formatter("function(params){" + "                    var date = new Date(params.value[0]);"
	...
	public List<Object[]> getData() {
		...
		for (int i = 0; i < 1500; i++) {
			dataList.add(new Object[] { getDateStr(new Date(114, 9, 1, 0, (int) Math.round(Math.random() * 10000))),
					(int) (round(Math.random() * 30) - 0), (int) (round(Math.random() * 100) - 0) });
		}
		return dataList;
	}
