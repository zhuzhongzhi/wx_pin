...
public class OfficeService extends TreeService<OfficeMapper, Office> {
	...
	public List<Office> getChildrenById(String parentId, String id, User user) {
		if (user.isAdmin()) {
			return officeMapper.findAllList(new Office());
		} else {
			/*
			 * Office office = new Office(); BaseService.dataRuleFilter(office);
			 * List<Office> officeList = officeMapper.findList(office);
			 */
			return officeMapper.getChildrenById(parentId, id);
		}
	}
