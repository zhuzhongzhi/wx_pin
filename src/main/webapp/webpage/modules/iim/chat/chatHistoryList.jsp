...
<link href="${ctxStatic}/plugin/layui/dist/css/layim.css"
		...
		<div class="layim-chat-main" style="width: 70%; height: 100%">
			<ul>
				<c:forEach items="${page.list}" var="chatHistory">
					<c:if test="${fn:contains(chatHistory.sender, '_msg_')}">
						<!-- 如果是群组信息，取出实际的发送者信息 -->
						<c:set var="sender"
							value="${fn:substringAfter(chatHistory.sender, '_msg_')}" />
					</c:if>
					<c:if test="${fn:contains(chatHistory.sender, '_msg_') == false}">
						<!-- 如果是群组信息，取出实际的发送者信息 -->
					...
					<c:if test="${fns:getUser().loginName != sender}">
						<li>
							<div class="layim-chat-user">
								<img src="${fns:getByLoginName(sender).photo}"> <cite>${fns:getByLoginName(sender).name}<i><fmt:formatDate
											value="${chatHistory.createDate}"
											pattern="yyyy-MM-dd HH:mm:ss" /></i></cite>
							</div>
							<div class="layim-chat-text">
								<c:if test="${fn:contains(chatHistory.msg, 'img[')}">
									<img
										src="${fn:substring(chatHistory.msg, 4, fn:length(chatHistory.msg)-1)}" />
								</c:if>
					...
					<c:if test="${fns:getUser().loginName == sender}">
						<li class="layim-chat-mine">
							<div class="layim-chat-user">
								<img src="${fns:getByLoginName(sender).photo}"> <cite><i><fmt:formatDate
											value="${chatHistory.createDate}"
											pattern="yyyy-MM-dd HH:mm:ss" /></i>${fns:getByLoginName(sender).name}</cite>
							</div>
							<div class="layim-chat-text">
								<c:if test="${fn:contains(chatHistory.msg, 'img[')}">
									<img
